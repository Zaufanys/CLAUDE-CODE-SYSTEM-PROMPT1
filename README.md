...
**Latest snapshot: `CC-SYS-PROMPT-07-11-26-*.md`** (parts 1–3). Older dated sets
(e.g. `CC-SYS-PROMPT-04-14-26-*.md`) are kept as historical records — the prompt
changes over time (model, tool set, environment), so don't treat an older file as
current. To capture a newer prompt, add another dated set; never edit an existing
one.
